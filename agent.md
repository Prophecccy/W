...
* **Strict Operational Constraints**:
  ...
  - **Secure Desktop & Web OAuth Flow (PKCE)**: Both Desktop and Web integrations strictly avoid embedding client secrets by implementing Proof Key for Code Exchange (PKCE) compliant with RFC 7636 (`code_challenge_method=S256`, `access_type=offline`).
    ...
    - **Web (Browser)**: `signInWithGoogleWeb()` uses PKCE authorization code grant (`response_type=code`, `access_type=offline`, `prompt=consent`). The authorization code is received via the tri-channel popup interceptor and exchanged directly at `https://oauth2.googleapis.com/token`. The returned `refreshToken` is securely stored in `localStorage`, eliminating the 60-minute session cliff and enabling uninterrupted background Google Drive synchronization indefinitely.
