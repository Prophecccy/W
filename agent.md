...
* **Strict Operational Constraints**:
  ...
  - **Real-Time Cross-Device Google Drive Sync Engine (`src/shared/services/localDb.ts`, `googleDriveService.ts`, `Layout.tsx`)**:
    - **Cross-Device Plain State Format**: `W_state.json` is persisted directly as standard formatted JSON in Google Drive (with backward-compatible support for legacy AES-GCM envelopes). Google Drive `W_state.json` is migrated from device-locked AES ciphertext to standard cross-device JSON so any client (Web or Desktop) can parse and merge state without device-specific keys.
    ...
    - **Lockout Resolution Precedence (`localDb.ts`)**: In `mergeObject()`, remote lockout resolutions (`strikes.current === 0`) take absolute priority over local desynced strike accruals, ensuring a lockout cleared on one device is immediately cleared across all connected clients.
