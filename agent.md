...
* **Strict Operational Constraints**:
  ...
  - **Local Production Build Standard**: Always compile standalone desktop binaries using the Tauri CLI (`npm run build:app` or `npx tauri build --no-bundle`). Never execute raw `cargo build --release` directly inside `src-tauri`; bare cargo builds bypass `beforeBuildCommand`, retain the development `devUrl` (`http://localhost:1420`), and omit embedding the `frontendDist` assets, causing offline startup crashes (`ERR_CONNECTION_REFUSED`).
  - **Native Desktop Security & IPC Boundary Defense**:
    - **IPC Command Validation**: All Tauri commands accepting frontend arguments (`update_sticky_regions`, `update_lockdown_blocklist`, `move_widget_by`, `update_lockdown_remaining`, `kill_blocked_process`) must strictly validate, clamp, and sanitize parameters inside native Rust before issuing Win32 OS calls.
    - **WebView2 Desktop CSP**: WebView2 windows enforce desktop Content Security Policy (`app.security.csp` in `tauri.conf.json`) to restrict script execution and external network connections to approved endpoints (Google OAuth/Drive, GitHub releases).
    - **Native Binary Minification & Obfuscation**: Release builds in `Cargo.toml` (`[profile.release]`) enforce `strip = true`, `lto = true`, `codegen-units = 1`, and `panic = "abort"` for full symbol stripping, dead-code elimination, and binary obfuscation.
    - **Win32 Hard Single-Instance Mutex**: Implements a global named Win32 mutex (`Global\com.wapp.desktop-primary-guard`) at the entry of `run()` before initializing Tauri plugins. If a secondary instance starts (via shortcut, autostart, or race condition), it attempts to signal the primary window via `WM_COPYDATA` or `SetForegroundWindow` and terminates immediately with `std::process::exit(0)`. This guarantees that two `w-app.exe` processes can never run concurrently under any startup race condition.
    - **Window Shadow Defense & Glass Border Prevention**: Transparent borderless windows (`widget`, `sticky-overlay`) in Windows 11 Desktop Window Manager (DWM) default to rendering an acrylic / frosted glass drop-shadow border frame if `shadow: false` is omitted. All window definitions in `tauri.conf.json` and any runtime window constructors (`WebviewWindow`) in `Layout.tsx` must explicitly declare `shadow: false`, and adhere to canonical dimensions (`width: 460, height: 620`), to preserve borderless, clean dark card aesthetics.
...
* **Strict Operational Constraints**:
  ...
  - **Desktop Widget Height & Scroll Bounds**:
    - **8-Item Viewport Ceiling**: The desktop widget displays a maximum of 8 habits simultaneously (`MAX_VISIBLE_WIDGET_HABITS = 8`). For $\le 8$ items, the widget dynamically sizes to fit that exact count. For $> 8$ items, the widget bounds clamp to the 8-item height, and remaining habits are reached through vertical scrolling.
    - **Pinned Footer & Scroll Isolation**: `.widget-app__right-panel` enforces `min-height: 0; overflow: hidden;` and `.widget-app__habits-scroll` enforces `flex: 1 1 auto; min-height: 0; overflow-y: auto; justify-content: flex-start; overscroll-behavior: contain;`. This guarantees that the bottom stats deck (`COMPLETED / TOTAL / PROGRESS`), footer status, and clock remain permanently pinned and visible on screen without clipping.
  - **Modal Architecture & Flex-Container Containment**:
    - **Wide Form Modal Standards**: When presenting wide configuration forms (`<HabitForm>`, `<TodoForm>`) inline within modal containers (such as `HabitsPage` modal or `PunishmentModal`), the modal container must enforce block flow (`display: block`) or explicit full-width alignment (`width: 95vw; max-width: 1200px;`) rather than relying on unconstrained flex column layouts.
    - **Intrinsic Grid Stretch Defense**: To prevent CSS Grid layouts (`repeat(auto-fill, minmax(...))`) like `<IconPicker>` from collapsing into intrinsic input min-widths (~200px / 3 icon columns), all intermediate wrappers (`.habit-form`, `.habit-form__content`, `.habit-form__step`, `.habit-form__step--appearance`, `.habit-form__field--full`, and `.icon-picker`) must declare explicit `width: 100%; box-sizing: border-box;`.
...
* **Strict Operational Constraints**:
  ...
  - **Secure Desktop & Web OAuth Flow (PKCE)**: Both Desktop and Web integrations strictly avoid embedding client secrets by implementing Proof Key for Code Exchange (PKCE) compliant with RFC 7636 (`code_challenge_method=S256`, `access_type=offline`).
    - **Desktop (Tauri)**: Prior to system browser redirection, a cryptographically secure random verifier string (`[A-Za-z0-9\-._~]`) is generated alongside its SHA-256 code challenge. The local loopback HTTP server captures the authorization code and exchanges it via POST to fetch access and refresh tokens.
    - **Web (Browser)**: `signInWithGoogleWeb()` uses PKCE authorization code grant (`response_type=code`, `access_type=offline`, `prompt=consent`). The authorization code is received via the tri-channel popup interceptor and exchanged through the secure Vercel Serverless Token Proxy (`/api/token`), which securely supplies `GOOGLE_CLIENT_SECRET` on the server without ever exposing it to client-side bundles. The returned `refreshToken` is stored in `localStorage`, and background token refreshes execute through `/api/token` with zero hourly expiration and zero security risk.
  - **Tri-Channel Web OAuth & Early Interceptor Architecture (`src/main.tsx`, `authService.ts`)**: To prevent OAuth redirect race conditions (where React Router mounts inside a popup and wipes the URL `#access_token` or `?code=` before the parent window reads it):
    1. **Early Popup Interceptor (`src/main.tsx`)**: Before React mounts, the application inspects `window.location.search` and `hash` for OAuth credentials (`code`, `access_token`, or `error`). If present in a popup (`window.opener` or `window.name === "google-login"`), it immediately relays credentials via `postMessage` (Channel 1) and writes a timestamped payload to `localStorage.setItem("w_oauth_response", ...)` (Channel 2), displays a minimal Endfield closing banner, triggers `window.close()`, and halts application mounting so React Router never wipes the query/hash or renders `/login` in the popup.
    ...
    3. **Route & Direct Callback Protection (`AuthGuard.tsx`, `useAuth.ts`)**: `<AuthGuard>` inspects `window.location` and holds on navigating to `/login` if `code=` or `access_token=` is detected. For single-tab or popup-blocked mobile browsers, `useAuth.ts` directly parses the callback on startup, exchanges the authorization code, and purges credentials from browser history using `window.history.replaceState()`.
  - **Real-Time Cross-Device Google Drive Sync Engine (`src/shared/services/localDb.ts`, `googleDriveService.ts`, `Layout.tsx`)**:
    - **Cross-Device Plain State Format**: `W_state.json` is persisted directly as standard formatted JSON in Google Drive (with backward-compatible support for legacy AES-GCM envelopes) so any client (Web or Desktop) can parse and merge state without device-specific keys.
    - **Pre-Gap Eager Synchronization (`Layout.tsx`)**: Before evaluating gaps or missed habits (`processGap()`), the application eagerly pulls and merges the latest consolidated state and notes from Google Drive. This prevents desynced clients from calculating false missed days and accruing phantom strikes when switching between devices.
    - **Lockout Resolution Precedence & State Merging (`localDb.ts`)**: In `mergeObject()`, remote lockout resolutions (`strikes.current === 0`) take absolute priority over local desynced strike accruals, synchronizing `merged.lastActiveDate = remote.lastActiveDate` to ensure a lockout cleared on one device is immediately cleared across all connected clients.
    - **Forced Pull on Local Lockout (`localDb.ts`)**: `pullAndMergeFromGoogleDrive(force)` bypasses the `w_gdrive_state_modified_time` optimization check if `force === true` or if `localUserDoc.strikes.current >= 5`. This guarantees that a locked client always pulls fresh remote data to verify if penance was already served on another device.
    - **Lockout Overlay Cloud Sync Action (`LockoutOverlay.tsx`)**: The lockout overlay features a direct `[ SYNC FROM GOOGLE DRIVE ]` recovery button with live connection/auth status indicators, allowing users to initiate an on-demand cloud sync and instantly unlock without completing redundant penance.
    - **Strike Deduplication & Sealed Lockout Resolution (`gapProcessor.ts`, `strikeService.ts`)**:
      1. `gapProcessor` indexes historical strikes from `userData.strikes.history` (`habitId:date`) and guards against adding duplicate strikes for the same habit on the same day.
      2. `resetStrikes()` atomically sets `"strikes.current": 0` AND synchronizes `"lastActiveDate"` to `today`. This seals the resolution so refreshing the page or restarting the app cannot trigger a retroactive gap run and re-strike the user.
    - **Deep Daily Log Merging (`localDb.ts`)**: To prevent sync races where an empty or partial log snapshot wipes completed tasks, `mergeLogsCollection()` performs deep field-level merging on daily logs (`mergeDailyLog()`). If a habit is marked completed in either local or remote records (`completed: true`), completion status is strictly preserved.
    - **Tombstone Defense Across Devices (`localDb.ts`)**: To prevent deleted habits from being resurrected by stale remote snapshots, `mergeCollection()` enforces deletion precedence: if an entity has `deleted: true`, the deletion tombstone is preserved across sync merges unless an active update occurs with a strictly newer timestamp.
    - **Sovereign Drive Protocol (Zero Firestore Dependency)**: Legacy Firestore auto-migration (`migrateFirestoreToLocal`) is permanently eradicated. Google Drive (`W_state.json`) is the sole authoritative state store; stale cloud Firestore data is never queried or permitted to overwrite local or cloud storage.
    - **Multi-Trigger Eager Synchronization**: Sync is driven by: (1) an active 10s state polling heartbeat and 15s note heartbeat, (2) immediate synchronization on `window.addEventListener("focus")` and `document.addEventListener("visibilitychange")` when switching between desktop and web tabs, (3) instant 1s debounced push on local mutations (`setDoc`, `updateDoc`, `deleteDoc`), (4) low-latency 2-second note sync deferral during typing, and (5) continuous background sync-down of remote Daily Notes (`pullNotesFromDrive()`).
    - **Reactive Note UI Refresh (`DailyNote.tsx`)**: When `w:note-synced` fires (with or without specific date detail), active notes are automatically re-read from IndexedDB into editor state if the user is not actively typing, ensuring multi-device notes reflect immediately without manual page reload.
...
* **Strict Operational Constraints**:
  ...
  - **Defense Against Client-Only Reliance (DAL Storage Gate & Rust IPC Boundary)**:
    - **DAL Storage Gate (`localDb.ts`)**: Never rely on UI validation alone. All write operations (`setDoc`, `updateDoc`, `addDoc`) must pass through `validateDocumentPayload()`, enforcing plain non-null objects, recursive prototype pollution elimination (`__proto__`, `constructor`, `prototype`), document size caps (500KB for standard entities, 5MB for logbook notes), and path safety checks before data can touch IndexedDB.
    - **Native Rust IPC Guards (`src-tauri/src/`)**: All Tauri commands receiving external or frontend parameters (e.g. `move_widget_by`, `update_lockdown_remaining`, `kill_blocked_process`) validate and clamp coordinate deltas, durations, and process IDs inside native Rust before issuing Win32 API calls.
  - **Dependency Hygiene & Zero-Vulnerability Standard**:
    - Regular audits via `npm audit` are strictly enforced to maintain 0 high/critical vulnerabilities.
    - Orphaned or high-vulnerability packages (e.g., `jimp`) are aggressively pruned from dependencies and devDependencies.
    - The repository strictly maintains a committed, reproducible `package-lock.json`.
  - **Zero-Hardcoded Secrets Standard**:
    - Secrets, tokens, and credentials must never be hardcoded in source code. All OAuth values consume `import.meta.env` or PKCE flows without secrets.
    - `.gitignore` rigorously excludes `*.env*` (except template `.env.example`), `*secret*`, `*credential*`, `*service-account*.json`, and certificate/key files (`*.pem`, `*.key`, `*.p12`, `*.pfx`).
    - Automated test assertions in `src/shared/utils/security.test.ts` statically scan the repository to detect accidental inclusion of Google API keys, private keys, or credentials.
  - **Production Code Minification, Obfuscation & Console Stripping (`vite.config.ts`)**:
    - Production builds must enforce `sourcemap: false` to ensure `.map` files are never generated or deployed, preventing source code reconstruction.
    - `esbuild` is configured to drop `console.*` and `debugger` calls (`drop: ['console', 'debugger']`) and remove all legal comments in production bundles.
    - Rollup chunk outputs use opaque content-hash filenames (`assets/[hash].js`) to prevent reverse engineering of internal routes and feature modularity.
...
## Batch 43 — Logbook Archive Restoration & Resilient Zero-Data-Loss Daily Notes

Resolved past daily notes disappearing in the Logbook archive (`/logbook`), permanently eliminated ephemeral key data loss on Web, and implemented automatic Google Drive sync-down and recovery:

1. **Persistent Web Encryption Key**:
   - In `src/shared/utils/noteCrypto.ts`, Web environments now store and retrieve the AES-GCM 256 key as JWK in IndexedDB under `w_note_crypto_key` with `localStorage` backup. Previously, non-Tauri environments generated an ephemeral non-extractable in-memory key on every reload, making all past notes undecryptable after refresh.
   - Added `ensureKey()` to automatically await key generation/import before `encryptNote()` and `decryptNote()`, eliminating startup race conditions.
2. **Resilient Local Log Service & Legacy Recovery**:
   - In `src/features/logs/services/localLogService.ts`, all read and write functions (`getLocalNoteHistory`, `readAndDecryptRecord`, `buildEncryptedRecord`) await `initEncryptionKey()`.
   - Added fallback to plaintext if decryption fails so records are never wiped or hidden.
   - Auto-scans `w_col_users/${userId}/logs` on logbook mount and seamlessly migrates historical user notes into IndexedDB `note_record_YYYY-MM-DD`.
   - Strictly filters out system placeholders (`[ AUTO-FREEZE ]`, `[ FROZEN ]`, `[ SYSTEM ]`).
3. **Broadened Google Drive Search & On-Demand Sync-Down**:
   - In `src/shared/services/googleDriveService.ts`, broadened file search query in `pullNotesFromDrive` to match `(mimeType = 'text/markdown' or mimeType = 'text/plain' or name contains '.md')`, fixing missed downloads for notes uploaded with non-markdown mime types.
   - Added `syncDownLogbookNotes()` for on-demand sync-down.
   - Emits `w:note-synced` global event so the Logbook UI updates in real time.
4. **Logbook Archive UI & Auto Sync-Down**:
   - In `src/features/logs/components/LogbookPage.tsx`:
     - Calls `initEncryptionKey()` on mount before loading logs.
     - Triggers background `syncDownLogbookNotes()` if Google Drive is linked.
     - Added manual `[ SYNC FROM DRIVE ]` button in the header with spinning animation and loading state.
     - Listens to both `w:note-saved` and `w:note-synced` events for instant UI synchronization.
5. **Daily Note Day Rollover & Content Protection**:
   - In `src/features/habits/components/DailyNote/DailyNote.tsx` & `DashboardPage.tsx`:
     - Passed `dailyResetTime` and computed logical `date` so Daily Note operates on the user's custom daily reset time (e.g. 04:00 AM) instead of midnight 00:00.
     - Prevented `initialNote` changes from destructively wiping active note content.
     - Loaded local note directly on mount from IndexedDB using the logical reset date.

---

## Batch 44 — Update-Proof Native OS Note Persistence & Storage Eviction Immunity

Permanently solved the issue of daily notes disappearing after Tauri desktop application updates:

1. **Root Cause Analysis**:
   - WebView2 (`AppData\Local\com.wapp.desktop\EBWebView`) manages IndexedDB under Chromium's `best-effort` quota policy. During application updates, the NSIS installer/updater, binary replacements, or process termination can cause WebView2 to reset local site storage, drop uncommitted LevelDB WALs, or treat previous database versions as abandoned.
   - In contrast, files stored on the native OS filesystem in `$APPDATA` (`AppData\Roaming\com.wapp.desktop`)—such as `note_key.json` and `backups/`—have survived 100% of all app updates without loss.
2. **Dual-Write Storage Engine (`$APPDATA/notes/` + IndexedDB)**:
   - In `src/features/logs/services/localLogService.ts`, implemented `saveNoteToDisk()` and `readNotesFromDisk()` using `@tauri-apps/plugin-fs` under `$APPDATA/notes/${date}.json`.
   - `saveLocalNote()`, `saveDownloadedNote()`, and `reEncryptLegacyRecord()` now write to IndexedDB **and** mirror the encrypted record directly to disk in `$APPDATA/notes/`.
   - `getLocalNoteHistory()` and `getLocalNoteRecord()` automatically scan `$APPDATA/notes/` and re-hydrate any missing notes into IndexedDB, making notes 100% immune to webview cache resets or installer updates.
3. **Capabilities Expansion**:
   - In `src-tauri/capabilities/default.json`, added `fs:allow-read-dir` and `fs:allow-remove` for `$APPDATA/**` to allow listing and managing the notes directory.
4. **Persistent Webview Storage Request**:
